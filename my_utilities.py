...
MODELO_DF = pd.DataFrame(
    data=[[
        'Esse será o nome de "Rótulo" que a aplicação usará, como por exemplo: Municípios, UBS, Escolas. Dado o fim da pesquisa.',
...
def generate_crunched_dataframes():
    with st.spinner("Carregando informações..."):
        ...
        # Verifica se os valores da coluna são majoritariamente reconhecidos como municípios
        mun_names = list(REGIONS_TABLE["Nome Município"].values)
        current_categories = st.session_state["base reader"].workable_database[tc[0]].unique()
        is_municipios = all(any(SequenceMatcher(None, unidecode(known_mun).lower(), unidecode(curr).lower()).ratio() >= 0.8 for known_mun in mun_names) for curr in current_categories)

        if is_municipios:
            # Procedimentos especializados para "Municípios"
            crunched_df = deepcopy(st.session_state["base reader"].workable_database)[tc + nc]
            crunched_df[tc[0]] = correct_city_names(crunched_df[tc[0]].values)
        else:
            crunched_df = deepcopy(st.session_state["base reader"].workable_database)[tc + nc]

        # Continue processamento independentemente de ser municípios ou não
        crunched_df[nc] = crunched_df[nc].clip(lower=0, upper=1)

        # Continuação do processamento original...
        ...
        for l in list_of_dfs:
            ...
            category_value = [l[tc[0]].values[0]]
            cat_avg = [float(np.average(flatlist(l[c].values))) for c in nc]
            cat_std = [float(calc_std(flatlist(l[c].values))) for c in nc]
            new_df_avg.append(category_value + cat_avg)
            new_df_std.append(category_value + cat_std)
